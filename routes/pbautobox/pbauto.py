...
class PBAuto:
    @staticmethod
    def connect_tcp(ip="", port=0, domain=0):
        return PBAuto(Tcp(ip, port, domain))
...
class Tcp(Connector):
    ...
    def __init__(self, ip, port=6211, domain=0):
        self.__port=port
        ...
        try:
            ...
            raise Exception("Couldn't connect to IP %s with Port %s", self.__ip, self.__port)
    ...
    def send(self, data, wait_for_response):
        ...
        # + protocol flag (byte, 0 for TCP)fo
        # + checksum (byte)
